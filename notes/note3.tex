...
\begin{frame}
  ...
  \item Numerical method such as the Newton-Raphson procedure can be used to find the value
    of $\hat\theta \equiv (\hat\lambda, \hat\gamma)$ that maximize the likelihood function. 
  \item The basic idea of the Newton-Raphson procedure iterates 
    $$\hat\theta_{n + 1} = \hat\theta_n + \left(-\frac{\dif^2\ell(\hat\theta_n)}{\dif\theta^2}\right)^{-1} \cdot \frac{\dif\ell(\hat\theta_n)}{\dif\theta}.$$
