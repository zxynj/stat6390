...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{> }\hlkwd{qplot}\hlstd{(}\hlkwd{log}\hlstd{(km}\hlopt{$}\hlstd{time),} \hlkwd{log}\hlstd{(}\hlopt{-}\hlkwd{log}\hlstd{(km}\hlopt{$}\hlstd{surv)))}
\end{alltt}
\end{kframe}
\end{knitrout}
  \begin{center}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{> }\hlstd{name} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"age"}\hlstd{,} \hlstr{"age:gender"}\hlstd{)}
\hlstd{> }\hlnum{2} \hlopt{-} \hlnum{2} \hlopt{*} \hlkwd{pnorm}\hlstd{(}\hlkwd{abs}\hlstd{(}\hlkwd{sum}\hlstd{(}\hlkwd{coef}\hlstd{(fit.exp)[name]))} \hlopt{/}
\hlstd{+ }              \hlkwd{sqrt}\hlstd{(}\hlkwd{sum}\hlstd{(fit.exp}\hlopt{$}\hlstd{var[name, name])))}
\end{alltt}
\begin{verbatim}
[1] 0.8584273
\end{verbatim}
