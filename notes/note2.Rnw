...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item $\p(t\le T< t + \dt|T\ge t)$ is a conditional probability.
      %the probability $T$ lies between $t$ and $t + \dt$ \emph{condition on} $T \ge t$.
...
\begin{frame}
  \frametitle{Counting processes}
  \begin{itemize}  
  \item Building onto our assumption, now assume we have independent $n$ subjects, 
...
\begin{frame}
  ...
  \item Assuming continuous survival times, and the aggregated processs jumps one unit at a time, 
    ...
    e.g., $Y(t) = n - N(t^-)$.
  \item Plug $\lambda(t)$ into Equation~\eqref{eq:Emart} gives an estimator for $H(t)$.
  \end{itemize}
\end{frame} 
