...
\begin{frame}
  \frametitle{Counting processes}
  \begin{itemize}  
  \item Now we will look at the scenario when there is only \empr{one event type} and a subject can experience \empr{at most one event}.
...
\begin{frame}
  \frametitle{Counting processes}
  \begin{itemize}  
  \item Building onto our assumption, now assume we have \empr{independent $n$ subjects},
...
\begin{frame}
  ...
  \item Intergrating both side after diving by $Y(t)$ gives
    ...
    \end{equation*}
  \item The right-hand side is a stochastic integral with respect to a 0-mean martingale.
  \item The first term in the left-hand side gives $\Hna(t)$.
  \item Then $\E\left\{\Hna(t) - \widehat{H}(t)\right\} = 0$ and $\Hna(t)$ is an unbiased estimator of $\widehat{H}(t)$.
  \item The variance estimator can be derived using the \emph{optional variation process}.
  \end{itemize}
\end{frame} 
...
\begin{frame}
  \frametitle{Counting processes}
  \begin{itemize}  
  \item For nonparametric or semi-parametric methods, we do not need to have a distributional assumption on the censoring time, $C$.
  ...
  \item Applying the independent censoring assumption, the intensity process for $N_i(t)$ takes the form $\lambda_i(t) = h_i(t) Y_i(t)$, where
...
\begin{frame}
  \frametitle{Counting processes}
  \begin{itemize}  
  \item As in the uncensored example, the \emph{aggregated} process is
    $$N(t) = \sum_{i = 1}^nN_i(t)= \sum_{i = 1}^n\I(\tilde{T}_i\le t, \Delta_i = 1),$$
    and the \emph{aggregated} intensity process is 
    $$ \lambda(t) = \sum_{i = 1}^n\lambda_i(t) = \sum_{i = 1}^nh_i(t)Y_i(t). $$
  \item In the case where $h_i(t) = h(t)$ for all $i$, the intensity process takes the form $\lambda(t) = h(t)Y(t)$,
