...
However, this is only a temporary fix, and the user might need to run this line everytime `devtools` is loaded. 

A potential reason for this issue is that `Rtools` is not being detected or not installed properly. 
For this, the user can try to install `Rtools` fro windows from this [link](https://cran.r-project.org/bin/windows/Rtools/).
